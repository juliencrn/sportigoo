...
class WC_Bookings_Customer_Meta_Box {
	...
	public function meta_box_inner( $post ) {
		...
		if ( $booking_order_id && $order ) {
			...
			<tr class="view">
				<th>&nbsp;</th>
				<td><a class="button button-small" target="_blank" href="<?php echo esc_url( admin_url( 'post.php?post=' . absint( $booking->get_order_id() ) . '&action=edit' ) ); ?>"><?php echo esc_html( 'View Order', 'woocommerce-bookings' ); ?></a></td>
			</tr>
			<?php
			$has_data = true;
		}
