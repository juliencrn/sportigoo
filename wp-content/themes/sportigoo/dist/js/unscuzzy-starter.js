...
var ajaxSearch = function ajaxSearch($) {
  ...
  var loader = outputSection.find('#js-loader');
  var refreshButton = searchSection.find('.js-refresh');
  ...
  if (urlParams.has('categorie')) {
    var val = urlParams.get('categorie');
  ...
  function callServer(argument) {
    var rm = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

    console.log({ argument: argument }); // debug
    ...
    $.post(ajaxurl, argument, function (res) {
  ...
  function handleFilter(selector) {
    var select = selector.parent().parent().find('select');
    ...
    switch (name) {
      ...
      case "what":
        ...
        args.product_cat = args.product_cat.filter(function (slug) {
          return !getSelectValues(name).includes(slug);
        });
