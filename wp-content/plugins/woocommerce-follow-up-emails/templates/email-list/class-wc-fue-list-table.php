...
class WC_FUE_List_Table extends WP_List_Table {
	...
	public function update_priorieties() {
		...
		foreach ( $types as $key => $type ) {
			if ( isset( $_REQUEST[ $key . '_order' ] ) && ! empty( $_REQUEST[ $key . '_order' ] ) ) {
				// Sanitized later
                // phpcs:ignore
	...
	public function column_priority( $email ) {
		...
		<input type="hidden" name= <?php echo esc_html( $email->type ); ?>_order[]" value="<?php echo esc_html( $email->id ); ?>" />
		<?php
	}
	...
	public function column_status( $email ) {
		...
		<span class="status-toggle">
			<?php if ( FUE_Email::STATUS_ACTIVE === $email->status ) : ?>
				...
				<br/><small><a href="#" class="toggle-activation" data-id="<?php echo esc_html( $email->id ); ?>"><?php esc_attr_e( 'Deactivate', 'follow_up_emails' ); ?></a></small>
			<?php elseif ( FUE_Email::STATUS_INACTIVE === $email->status ) : ?>
				<?php esc_attr_e( 'Inactive', 'follow_up_emails' ); ?>
				<br/><small><a href="#" class="toggle-activation" data-id="<?php echo esc_html( $email->id ); ?>"><?php esc_attr_e( 'Activate', 'follow_up_emails' ); ?></a></small>
			<?php else : ?>
				<?php esc_attr_e( 'Archived', 'follow_up_emails' ); ?>
				<br/><small><a href="#" class="unarchive" data-id="<?php echo esc_html( $email->id ); ?>"><?php esc_attr_e( 'Unarchive', 'follow_up_emails' ); ?></a></small>
			<?php endif; ?>
		</span>
		<?php if ( FUE_Email::STATUS_ARCHIVED != $email->status ) { ?>
			|
			<small><a href="#" class="archive-email" data-id="<?php echo esc_html( $email->id ); ?>" data-key="<?php echo esc_html( $email->type ); ?>"><?php esc_attr_e( 'Archive', 'follow_up_emails' ); ?></a></small>
		<?php } ?>
		<?php
...
?>
	...
	protected function extra_tablenav( $which ) {
		...
		<div class="alignleft actions">
            <input type="hidden" name="view" value="<?php echo esc_html( $this->view ); ?>">
